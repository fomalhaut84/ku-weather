...
- `fix/<issue-number>-<n>`: 버그 수정. dev에서 생성 → dev로 PR.
- `hotfix/<issue-number>-<n>`: 프로덕션 버그. main에서 생성 → main, dev 양쪽 머지.
...
- dev → main 머지 후 태그 생성.
- 태그 형식: `v{major}.{minor}.{patch}` (예: `v2.0.0`)
...
gh release create v2.0.0 --title "v2.0.0" --notes "릴리즈 노트 내용"
```

버전 기준:
- major: 큰 아키텍처 변경 또는 Breaking change
- minor: 기능 추가 (이슈 단위)
- patch: 버그 수정, hotfix
...
```
라벨: `feature`/`bug`/`chore`, 우선순위 `P0`/`P1`/`P2`
...
### 9. PR 생성
```bash
gh pr create --title "feat: 기능명 (#<issue>)" --base dev --head feat/...
```
