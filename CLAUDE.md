...
- `npm test` - 테스트 실행 (현재 기본 오류 메시지 출력)
...
#### 1. 테스트 프레임워크 설정
- ✅ **Jest 설치 및 설정** - TypeScript 지원, 커버리지 포함
- ✅ **TypeScript 테스트 환경 구성** - ts-jest 프리셋 사용
- ✅ **테스트 실행 스크립트 추가** - `npm test`, `npm run test:watch`, `npm run test:coverage`, `npm run test:ci`
- ✅ **코드 커버리지 설정** - HTML, LCOV, 텍스트 리포트 생성

#### 2. 단위 테스트 작성 (진행 중)
- ✅ **WeatherService 기본 테스트 구조** - 테스트 파일 생성됨
- [ ] **WeatherService 상세 테스트**
  - API 호출 성공/실패 시나리오
  - CSV 파싱 로직 테스트  
  - 지역명 매핑 테스트
  - 특보 필터링 로직 테스트
...
#### 3. 테스트 설정 파일들
- ✅ **jest.config.js** - Jest 설정 (타임아웃, 커버리지, 패턴 등)
- ✅ **jest.setup.js** - 테스트 환경 초기화
- ✅ **커버리지 리포트** - HTML 형태로 생성됨

### TODO: 남은 작업들

#### 1. 단위 테스트 완성
- [ ] WeatherService 테스트 케이스 완성
- [ ] SlackService 테스트 작성
- [ ] Config 테스트 작성

#### 2. 통합 테스트
- [ ] API 통합 테스트 (실제 기상청 API 호출)
- [ ] Slack 통합 테스트 (테스트용 웹훅)
- [ ] 전체 모니터링 플로우 테스트
